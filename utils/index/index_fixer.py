...
class IndexFixer:
    ...
    @staticmethod
    def _prefix_zeroes(file: File, main_index: int) -> str:
        """This ensures that all indexes in one directory are the same length"""
        if ih.is_category(
            file.get_parent(), proper=True
        ):  # Topics are special where we want the main index to have 2 digits (Eg: 12.01)
    ...
    @staticmethod
    def _compute_new_main_index(file: File) -> str:
    ...
    @staticmethod
    def _compute_parent_index(child_file: File) -> str:
        parent_file = child_file.get_parent()
        if ih.is_area(parent_file, proper=True):
            parent_index = ih.get_main_index(parent_file)
